# -*- coding: utf-8 -*-
"""
Created on Fri Nov  4 09:13:05 2022
...
low, high = -3, 3
x_vals = np.random.default_rng().uniform(low,high,100)
x = torch.tensor(x_vals, requires_grad=True)
x = x.float()
x = x[torch.randperm(len(x))]
x_valid = np.random.default_rng().uniform(low,high,10)
...
for epoch in range(num_epochs):
    ...
    if valid_err_cur < eps:
        print(f'Broke because validation below error. Number epochs: {epoch}')
        break
