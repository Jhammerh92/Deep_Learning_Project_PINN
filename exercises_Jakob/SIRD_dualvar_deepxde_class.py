...
class SIRD_deepxde_net:
    def __init__(self, t, wsol, alpha_guess=0.1, beta_guess=0.1, gamma_guess=0.1,
        ...
        def pde(t, y):
            ...
            alpha_a  = torch.sqrt(self.alpha_a**2)
            alpha_b  = torch.sqrt(self.alpha_b**2)
            alpha_aa = torch.sqrt(self.alpha_aa**2)
            alpha_bb = torch.sqrt(self.alpha_bb**2)
            alpha_ba = torch.sqrt(self.alpha_ba**2)
            alpha_ab = torch.sqrt(self.alpha_ab**2)
            beta_a   = torch.sqrt(self.beta_a**2)
            beta_b   = torch.sqrt(self.beta_b**2)
    ...
    def _get_best_params(self):
        ...
        df[7] = np.sqrt(df[7].str[:-1].astype('float')**2)
        df[8] = np.sqrt(df[8].str[:-1].astype('float')**2)
        df[9] = np.sqrt(df[9].str[:-1].astype('float')**2)
        df[10] = np.sqrt(df[10].str[:-1].astype('float')**2)
        df = df.loc[self.train_state.best_step]
        # self.best_alpha_nn, self.best_beta_nn, self.best_gamma_nn = df[1], df[2], df[3]
        self.best_alpha_a_nn, self.best_alpha_b_nn, self.best_alpha_aa_nn, self.best_alpha_bb_nn, self.best_alpha_ba_nn, self.best_alpha_ab_nn, self.best_beta_a_nn, self.best_beta_b_nn, self.best_gamma_a_nn, self.best_gamma_b_nn = df[1], df[2], df[3], df[4], df[5], df[6], df[7], df[8], df[9], df[10]
        return self.best_alpha_a_nn, self.best_alpha_b_nn, self.best_alpha_aa_nn, self.best_alpha_bb_nn, self.best_alpha_ba_nn, self.best_alpha_ab_nn, self.best_beta_a_nn, self.best_beta_b_nn, self.best_gamma_a_nn, self.best_gamma_b_nn
