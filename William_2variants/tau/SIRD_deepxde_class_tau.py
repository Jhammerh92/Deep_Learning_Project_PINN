...
class SIRD_deepxde_net:
    def __init__(self, t, wsol, alpha_guess=0.1, beta_guess=0.1, gamma_guess=0.1,
        ...
        self.alpha = dde.Variable(alpha_guess)
        self.tau = dde.Variable(beta_guess)
        self.gamma = dde.Variable(gamma_guess)
        ...
        def pde(t, y):
            ...
            alpha  = (self.alpha)/(self.tau)
            # print(f'self: {self.alpha}, alpha: {alpha}, tau: {self.tau}')

            beta   = (self.alpha)/(self.tau**2)
        ...
        self.variables = [self.alpha, self.tau, self.gamma]
